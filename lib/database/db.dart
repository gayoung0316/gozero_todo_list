...
class DBHelper {
  ...
  Future<Database> get database async {
    ...
    _db = openDatabase(
      ...
      onCreate: (db, version) {
        return db.execute(
          "CREATE TABLE todo(id TEXT PRIMARY KEY, title TEXT, priority INTEGER, success INTEGER, color INTEGER, date TEXT)",
        );
      },
      // 버전을 설정하세요. onCreate 함수에서 수행되며 데이터베이스 업그레이드와DateFormat('MM월 dd일').format(DateTime.now()).toString() 다운그레이드를
      // 수행하기 위한 경로를 제공합니다.
      version: 2,
    );
    return _db;
  ...
  Future<ToDo> insertMemo(ToDo todo) async {
  ...
  Future<void> updateMemo(ToDo todo) async {
    final db = await database;
